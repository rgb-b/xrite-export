...
var STEP_COL_W       = 30;    // pt — left gutter (step-% labels)
...
/** Add a point-sized text frame at (x, y) with given width and string content.
 *  y is measured from the TOP of the artboard (script converts to Illustrator
 *  bottom-left origin internally).
 *  Returns the TextFrame object. */
function addText(layer, x, yFromTop, w, h, content, fontSize, bold) {
    var tf = layer.textFrames.add();
    tf.geometricBounds = [yFromTop, x, yFromTop + h, x + w];  // [top, left, bottom, right]
    tf.contents = content;

    var attr = tf.textRange.characterAttributes;
    attr.size = fontSize || FONT_SIZE;
    try {
        attr.textFont = app.textFonts.getByName(bold ? FONT_FAMILY + "-Bold" : FONT_FAMILY);
    } catch (e) {
        // font not found — leave default
    }
    return tf;
}

/** Draw a hairline rectangle (no fill, 0.25pt stroke). */
function addRect(layer, x, yFromTop, w, h) {
    var r = layer.pathItems.rectangle(
        -yFromTop,        // Illustrator top (negative = down from origin at bottom-left)
        x,
        w,
        h
    );
...
function buildTemplate(stepLabels, outFileName) {
    ...
    doc.artboards[0].artboardRect = [0, PAGE_H, PAGE_W, 0];  // [left, top, right, bottom] in Illustrator coords
    ...
    // (recalc STEP_COL_W to fill exactly)
    ...
    // Row Y positions from TOP of artboard (positive downward in our coordinate convention)
    ...
    // step rows start after density row
    // row i (0-based) → yDensity + ROW_H + i * ROW_H

    // ── 3. Header block ──────────────────────────────────────────────────────
    // Row 1: CUSTOMER (left, wide) | CRS (mid) | DATE | SET | JOB
    var hRowH = HEADER_H / 2;

    // CUSTOMER spans about half the width
    ...
    // ── 4. Step-label gutter header (blank) ─────────────────────────────────
    // (no text needed in gutter header cells — step values go in data rows)
    ...
    // ── 6. Step-label gutter (left column) ───────────────────────────────────
    // Density row label
    ...
    // ── 7. Outer border for the data table ───────────────────────────────────
